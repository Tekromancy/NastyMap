...
async function main() {
    ...
    if (cli.flags.diff && Array.isArray(cli.flags.diff) && cli.flags.diff.length >= 2) {
        ...
        const sampleKey = cli.flags.sample;
    ...
    const initialTab = cli.flags.tab || (cli.flags.diff ? 'diff' : 'topology');
    // 3. Render Ink TUI Application
    render(_jsx(App, { initialScan: scan, initialScanLabel: scanLabel, initialTab: initialTab, diffScanA: diffScanA, diffScanB: diffScanB }));
}
main().catch((err) => {
    console.error(err);
    process.exit(1);
